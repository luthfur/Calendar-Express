...
function submitForm() {
	...
	event_descObj = document.getElementById('event_desc');
	
	if(isRichText) {
		event_descObj.value = descObj.contentWindow.document.body.innerHTML;
	
	} else {

		event_descObj.value = descObj.value;
		event_descObj.text = descObj.text;

	}

	//alert('desc = ' + event_descObj.value);
