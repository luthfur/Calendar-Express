...
class EventData {
	...
	function getDetails( $id = null, $typeid = null, $cid = null, $uid = null, $unapp=null ) {
		...
		$query = "SELECT * FROM $event, $calendar, $occurence, $user, $location, $contact WHERE $event.type_id = $occurence.type_id AND $calendar.calendar_id = $event.calendar_id AND $user.user_id = $event.user_id AND $location.location_id = $event.location_id AND $contact.contact_id = $event.contact_id AND $event.approved = 1";
